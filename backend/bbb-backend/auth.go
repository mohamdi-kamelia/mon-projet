...
func handleRegister(w http.ResponseWriter, r *http.Request) {
	...
	result, err := db.Exec(
		...
		req.Email, string(hashedPassword), req.Name, "visiteur", "online",
	)
	if err != nil {
...
func handleLogin(w http.ResponseWriter, r *http.Request) {
	...
	err := db.QueryRow(
		"SELECT id, email, password_hash, name, role, status, avatar_config, created_at, last_login FROM users WHERE email = ?",
		req.Email,
	).Scan(&user.ID, &user.Email, &user.PasswordHash, &user.Name, &user.Role, &user.Status, &avatarConfig, &user.CreatedAt, &lastLogin)
	if err != nil {
	...
	now := time.Now()
	db.Exec("UPDATE users SET last_login = ?, status = 'online' WHERE id = ?", now, user.ID)
	user.LastLogin = &now
	user.Status = "online"
	...
	log.Printf("✅ User logged in: %s (ID: %d, role: %s)", user.Email, user.ID, user.Role)
...
func handleGetMe(w http.ResponseWriter, r *http.Request) {
	...
	err := db.QueryRow(
		"SELECT id, email, name, role, status, avatar_config, created_at, last_login FROM users WHERE id = ?",
		userID,
	).Scan(&user.ID, &user.Email, &user.Name, &user.Role, &user.Status, &avatarConfig, &user.CreatedAt, &lastLogin)
	if err != nil {
		if err == sql.ErrNoRows {
...
func handleUpdateProfile(w http.ResponseWriter, r *http.Request) {
	...
	if updates.Status != nil {
		valid := map[string]bool{"online": true, "away": true, "dnd": true}
		if !valid[*updates.Status] {
			respondError(w, "Invalid status, must be: online, away, dnd", http.StatusBadRequest)
			return
		}
	}
	...
	log.Printf("✅ Profile updated: User ID %d (status: %s)", userID, user.Status)
	respondJSON(w, user, http.StatusOK)
}
...
func handleForgotPassword(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respondJSON(w, SuccessResponse{Message: "If the email exists, a reset link has been sent"}, http.StatusOK)
		return
	}
